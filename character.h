...
enum class CharacterType {
    WARRIOR, ARCHER, WIZARD, GOBLIN, GOBLIN_CAPTAIN, GOBLIN_KING
};
...
public:
    ...
    void buffStrength(double multiplier) {
    ...
    // Metodos abstractos: Character (clase padre de Hero y Enemy) solo declara el
    // contrato; cada tipo concreto (Warrior, Archer, Wizard, Goblin, GoblinCaptain,
    // GoblinKing) lo implementa, incluyendo getType() con su propio CharacterType.
...
protected:
    double typeMultiplier(Character* target) {
        return damageMultiplier(getType(), target->getType());
    }

    // Se llama desde basicAttack() cuando el golpe es critico. Character no
    // hace nada por defecto; Hero la sobrescribe para curarse con cada
    // critico (ver onCriticalHit en hero.h).
    virtual void onCriticalHit(int damage) {}

private:
    static double damageMultiplier(CharacterType attacker, CharacterType defender) {
        if (attacker == CharacterType::WARRIOR && defender == CharacterType::GOBLIN) return 1.2;
        if (attacker == CharacterType::GOBLIN && defender == CharacterType::WARRIOR) return 0.8;

        if (attacker == CharacterType::WARRIOR && defender == CharacterType::GOBLIN_CAPTAIN) return 1.2;
        if (attacker == CharacterType::GOBLIN_CAPTAIN && defender == CharacterType::WARRIOR) return 0.8;

        if (attacker == CharacterType::GOBLIN_KING && defender == CharacterType::WARRIOR) return 1.2;
        if (attacker == CharacterType::WARRIOR && defender == CharacterType::GOBLIN_KING) return 0.8;

        if (attacker == CharacterType::ARCHER && defender == CharacterType::GOBLIN_CAPTAIN) return 1.2;
        if (attacker == CharacterType::GOBLIN_CAPTAIN && defender == CharacterType::ARCHER) return 0.8;

        if (attacker == CharacterType::GOBLIN_KING && defender == CharacterType::ARCHER) return 1.2;
        if (attacker == CharacterType::ARCHER && defender == CharacterType::GOBLIN_KING) return 0.8;

        if (attacker == CharacterType::GOBLIN && defender == CharacterType::WIZARD) return 1.2;
        if (attacker == CharacterType::WIZARD && defender == CharacterType::GOBLIN) return 0.8;

        if (attacker == CharacterType::GOBLIN_CAPTAIN && defender == CharacterType::WIZARD) return 1.2;
        if (attacker == CharacterType::WIZARD && defender == CharacterType::GOBLIN_CAPTAIN) return 0.8;

        if (attacker == CharacterType::WIZARD && defender == CharacterType::GOBLIN_KING) return 1.2;
        if (attacker == CharacterType::GOBLIN_KING && defender == CharacterType::WIZARD) return 0.8;
